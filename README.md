...
Библиотека SequenceLib v.4.0 представляет собой набор макросов для PlantUML, упрощающих создание сложных sequence-диаграмм с:

* Автогенерацией документации напрямую из кода диаграммы, исключая расхождения.
* Стандартизированным оформлением (цвета, шрифты, активации)
* Связью с техническими требованиями через таблицы
...
Модель становится основой для ТЗ разработчикам (ошибки, параметры, сценарии). Сокращается объем рукописного кода на 40-60% за счет макросов.
...
Для использования библиотеки релиз, в разделе RESOURCES расположен архив с шаблонами [PlantUmlTemplates.zip](SequenceLib%2FRESOURCES%2FPlantUmlTemplates.zip), повторяющими наименования процедур библиотеки.  
Импортируйте их в свою среду разработки о [инструкции](https://www.jetbrains.com/help/idea/sharing-live-templates.html#export-and-import-live-templates-manually) для более комфортного взаимодействия с библиотекой.
...
1. Кастомизация участников: 
2. Кастомизация событий аудита:
3. Кастомизация конфигурационных параметров:
4. Кастомизация ошибок:
5. Кастомизация метрик:
6. Кастомизация типов участников:
...
вызовет процедуру todo() и создаст строку с задачей на разработку, также добавит строку с предзаполненным описанием ошибки.     
...
В Легенде создаются таблицы только с новыми элементами. В примере не создаются новые события аудита или метрики, поэтому эти таблицы отсутствуют в легенде.