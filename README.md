...
Библиотека SequenceLib v.5.0 представляет собой набор макросов для PlantUML, упрощающих создание sequence-диаграмм с набором функций:
...
![img_3.png](SequenceLib/RESOURCES/ASIS.png)


Пример диаграммы последовательности TO BE (с применением библиотеки)  
![img_3.png](SequenceLib/RESOURCES/TOBE.png)

Благодаря встроенной функции PlanntUML, позволяющей сгенерировать диаграмму в формате unicode ASCII,  
мы можем получить доступ к сгенерированной автоматический документации. Для этого кликните по заметке с оригинальным синтаксисом
правой кнопкой мыши, и скопируйте диаграмму в формате UNICODE ASCII 
![img.png](SequenceLib/RESOURCES/unicodeascii.png)

В верхней части будет диаграмма в юникоде, а ниже, в заметке - код в оригинальном синтаксисе:
![img.png](SequenceLib/RESOURCES/originalCode.png)

На текущий момент конвертер на завершаюшей стадии готовности, можете ознакомиться в [репозитории.](https://github.com/leraRadicovich/SequnceLibConverter)

После извлечения данных вы получаете документацию в формате markdown/adoc   
![img_3.png](SequenceLib/Documentation/legend.png)


## С чего начать?
### Установка библиотеки
Библиотека совместима с версией PlantUml 1.2024.7 и выше, и успешно протестирована в плагине для Intellij IDEA версии 7.13.0-IJ2023.2.   
Для проверки установленной версии введите команду version в любой PlantUML-диаграмме.  
Для корректной работы отключите в настройках удаленный рендеринг. Перейдите в настройки
![img.png](SequenceLib/RESOURCES/img.png)

Снимите галку remote rendering
![img_1.png](SequenceLib/RESOURCES/img_1.png)

1. Скачайте архив c последним релизом библиотеки из GIT-репозитория.
2. Распакуйте архив в выбранной папке у себя на компьютере.
3. Создайте диаграмму plantUML с помощью установленного плагина (Рекомендую использовать [версию 7.13.0-IJ2023.2](https://plugins.jetbrains.com/plugin/7017-plantuml4idea/versions/stable/785787)).
...
В распакованном релизе, в разделе RESOURCES расположен архив с шаблонами [umlLiveTemplates.zip](SequenceLib%2FRESOURCES%2FumlLiveTemplates.zip), повторяющими наименования процедур библиотеки.  
...
### Изучение на прмиере
Если Вам легче обучаться на примере, рекомендую просто открыть демонстрационный-файл:[demoFile.puml](SequenceLib%2FDocumentation%2FdemoFile.puml)
Не забудьте обновить в демонстрационном файле путь до файла библиотеки [SequenceLibIncludeFile_v5.puml](SequenceLib%2Fsrc%2FSequenceLibIncludeFile_v5.puml) у себя на компьютере.
...
![img_2.png](SequenceLib/RESOURCES/img_2.png)
